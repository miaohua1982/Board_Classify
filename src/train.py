...
label_path = "/root/board/7.20"
img_base_path = "/root/board/7.20"
...
def get_train_validate_dataloader_kflod(train_idx, test_idx, ds, batch_size=64, alb=False):
    ...
    if alb:
        trainset = BoardDatasets_Alb2(train_ds, alb_train_transform)
        validateset = BoardDatasets_Alb2(test_ds, alb_valid_transform)
    else:
        trainset = BoardDatasets2(train_ds, img_base_path, train_transform)
        validateset = BoardDatasets2(test_ds, img_base_path, train_transform)
...
def get_train_validate_dataloader(img_base_path, label_path, batch_size=4, alb=False):
    train_transform = transforms.Compose(
    ...
    if alb:
        trainset = BoardDatasets_Alb(img_base_path, label_path, alb_train_transform, ratio=0.8, is_train=True)
        validateset = BoardDatasets_Alb(img_base_path, label_path, alb_valid_transform, ratio=0.8, is_train=False)
    else:
        trainset = BoardDatasets(img_base_path, label_path, train_transform, ratio=0.8, is_train=True)
        validateset = BoardDatasets(img_base_path, label_path, valid_transform, ratio=0.8, is_train=False)

    trainloader = t.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, drop_last=False)
    validateloader = t.utils.data.DataLoader(validateset, batch_size=batch_size, shuffle=False, drop_last=False)
...
def model_train(net, trainloader, criterion, optimizer, scheduler, epoch, spot_step, mixup_fn=None, MIXUP=False):
    ...
    for i, data in enumerate(trainloader):
        ...
        total_acc += (outputs.argmax(dim=1) == labels).float().sum().cpu().item()
        counter += outputs.shape[0]
...
def train(classes_num, spot_step, model_path):
    ...
    net = model_board_timm(classes_num).to(device)
    ...
    train_criterion = tu.SoftTargetCrossEntropy().to(device)
    ...
    for epoch in range(epochs):
        ...
        t.save(net.state_dict(), 'model_storage/model_board_%s_%d_%.6f.pkl' % (time.strftime('%Y-%m-%d',time.localtime(time.time())), epoch+1, val_acc))
...
def train_kflod(classes_num, spot_step, model_path):
    ...
    train_idxs, test_idxs, ds = get_n_splits(img_base_path, label_path, kflod)
    for train_idx, test_idx in zip(train_idxs, test_idxs):
        ...
        if model_path is None:
            # net = model_board_py(classes_num)
            net = model_board_timm(classes_num)
        else:
